...
use crate::synthesis::events::SynthesisInvocationEvent;
use crate::synthesis::model::{ModelContext, OutputCandidate, SynthesisModel};
use crate::synthesis::planner::SynthesisPlanStep;
use crate::synthesis::tool_protocol::{ToolCallResult, ToolProtocol};

pub use crate::synthesis::events::ContinuationState;
pub use crate::synthesis::invocation::{InvocationContext, InvocationState, ResumeAction};
...
pub enum PendingAction {
    ToolRequest {
        request: crate::synthesis::tool_protocol::ToolCall,
    },
}
    ...
    loop {
        ...
        match step {
            SynthesisPlanStep::RequestTool(tool_call) => {
                if state.continuation == ContinuationState::AwaitingTool {
                    match resume_action.take() {
                        Some(ResumeAction::ToolCompleted) => {
                            state
                                .events
                                .push(SynthesisInvocationEvent::ContinuationAdvanced {
                ...
                state.events.push(SynthesisInvocationEvent::ToolRequested {
            ...
            SynthesisPlanStep::ProduceOutputCandidate => {
                ...
                match model.produce_output_candidate(&context) {
                    Ok(output_candidate) => {
                        state
                            .events
                            .push(SynthesisInvocationEvent::OutputCandidateReturned {

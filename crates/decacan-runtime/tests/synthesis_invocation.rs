use decacan_runtime::gateway::{SynthesisGatewayAdapter, ToolGateway};
use decacan_runtime::policy::entity::PolicyProfile;
use decacan_runtime::unstable_synthesis::{
    ...
    PendingAction, ResumeAction, SynthesisModel, ToolCall, ToolCallResult, ToolProtocol,
};
use serde_json::{from_str, to_string};

#[test]
fn synthesis_invocation_requests_tool_and_returns_output_candidate() {
    let gateway = ToolGateway::new(
        PolicyProfile::new_for_test("policy-1", "workspace-1", "default"),
        "/tmp/workspace/output",
    );
    let tool_protocol = SynthesisGatewayAdapter::new(gateway);
    let model = SummaryModelForTest;
    let context = InvocationContext {
        source_material: "source material collected outside synthesis".to_owned(),
...
#[test]
fn synthesis_invocation_blocks_when_tool_requires_approval() {
...
#[test]
fn synthesis_invocation_blocks_when_tool_is_denied() {
...
#[test]
fn synthesis_invocation_fails_when_tool_protocol_errors() {
...
#[test]
fn synthesis_invocation_fails_when_model_fails() {
...
#[test]
fn synthesis_invocation_state_roundtrips_and_resumes_after_tool_completion() {
    let tool_protocol = StubToolProtocol::allowed("allowed");
    let context = invocation_context();
...
#[test]
fn synthesis_adapter_returns_policy_allow_without_fake_payload() {
    let gateway = ToolGateway::new(
        PolicyProfile::new_for_test("policy-1", "workspace-1", "default"),
        "/tmp/workspace/output",
    );
    let adapter = SynthesisGatewayAdapter::new(gateway);
...
#[test]
fn synthesis_adapter_preserves_blocking_policy_decisions() {
    let gateway = ToolGateway::new(
        PolicyProfile::new_for_test("policy-1", "workspace-1", "default"),
        "/tmp/workspace/output",
    );
...
impl SynthesisModel for SummaryModelForTest {
    type Error = ();
...
impl SynthesisModel for FailingModel {

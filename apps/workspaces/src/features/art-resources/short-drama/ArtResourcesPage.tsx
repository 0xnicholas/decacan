 ...
 * - 'content' (short-video): Visual asset library - templates, subtitles, filters
 * - 'production' (short-drama): Art resource library - character designs, scenes, props
 */
export function ArtResourcesPage() {
  ...
  const categories = isContentMode ? contentCategories : productionCategories;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div key={i} className="aspect-square rounded-lg border bg-card overflow-hidden group cursor-pointer">
                  ...
                  <div className="h-1/4 p-3">
                    <div className="flex items-center justify-between">
                      ...
                      {'color' in category ? (
                        <span className={`text-xs px-2 py-0.5 rounded ${(category as { name: string; count: number; color: string }).color}`}>
                          {category.name}
                        </span>
                      ) : (
        ...
        <div className="space-y-4">
          <div className="rounded-lg border bg-card p-4">
            <h3 className="font-semibold mb-3">{isContentMode ? '素材统计' : '资源统计'}</h3>
